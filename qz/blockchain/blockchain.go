...
type Signature struct {
	ContentHash cas.HashData `json:"contentHash"` // for chain head it's chainhead hash
	Jws         []byte       `json:"jws"`
...
type BlockChain struct {
	...
	PrevChainHash cas.HashData `json:"prevChainHash"` // genesis block previous hash is Hash(ChainID)
...
type BlockTxn struct {
	...
	TxnHash    cas.HashData `json:"txnHash"`
	TxnFamily  cas.HashData `json:"txnFamilyHash"` // Hash of ClassName
...
func (btx *BlockTxn) MerkleHasher() map[string]merkle.Hasher {
	...
	m["txnTmstamp"] = cas.NewHashData([]byte(btx.TxnTmstamp.String()))
	return m
}
